...
function initGame() {
    H = parseInt(document.getElementById('input-size').value);
    W = parseInt(document.getElementById('input-size').value);
    if (H > 100 || W > 100) { console.log("The size is too big."); return; }
    else if (H == 1 || W == 1) { console.log("The size is too small."); return; }
    max_strength = H + W - 2;
...
function endTurn() {
    ...
    if (result !== null) {
        setTimeout(() => {
            alert(`ゲーム終了！ ${result.p1Score} vs ${result.p2Score} で ${result.winner} の勝ち！ (Winner : ${result.winner})`);
        }, 100);
    }
}
